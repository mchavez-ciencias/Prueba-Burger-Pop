...
class FPF_Product {
    ...
    public function create_field( array $field, WC_Product $product ) {
        $field_type  = $this->get_field_type( $field['type'] );
		$field_key   = $field['id'];
		$field_value = $field_type['type_object']->get_field_value( $field_key, true );
		$args        = ( new TemplateArgs() )->parse_field_args(
			$field_type,
        ...
        if ( $field['type'] == 'heading' ) {
		    ...
		    $ret = $this->_plugin->load_template( 'date', 'fields', array( 'key' => $field_key, 'args' => $args, 'value' => $field_value, 'type' => $field['type'] ) );
	    }
        else if ( $field['type'] == 'radio' ) {
            add_filter( 'woocommerce_form_field_radio', array( $this, 'woocommerce_form_field_radio' ), 10, 4 );
	        $ret = $this->_plugin->load_template( $field['type'], 'fields', array( 'key' => $field_key, 'args' => $args, 'value' => $field_value, 'type' => $field['type'] ) );
