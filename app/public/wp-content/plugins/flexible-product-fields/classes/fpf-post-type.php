...
//                'show_in_menu'		  => 'product',
    ...
    public function manage_fpf_fields_posts_custom_column( $column ) {
    	...
    	switch ( $column ) {
			case 'fpf_fields':
				...
				$labels = array_column( $values, 'title' );
			...
			case 'fpf_assign_to':
				...
				$meta_key = $assign_to_options[ $assign_to ]['meta_key'];
				if ( ! $meta_key ) {
    ...
    public function settings_meta_box_output( $post ) {
		...
		$fpf_settings['tags'] = array( 'value' => get_post_meta( $post->ID, '_tags', true ) );
		if ( !is_array( $fpf_settings['tags'] ) ) {
			$fpf_settings['tags'] = array( 'value' => array() );
		}
