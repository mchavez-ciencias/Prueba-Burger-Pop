...
const getProductsOptions = (input) => {
	return fetch(
		fpf_admin.rest_url + 'flexible_product_fields/v1/products/' + fpf_admin.rest_param + 'search=' + input,
		{
			headers: {
...
let getCategoriesOptions = (input) => {
	return fetch(
		fpf_admin.rest_url + 'flexible_product_fields/v1/categories/' + fpf_admin.rest_param + 'search=' + input,
		{
			headers: {
				'X-WP-Nonce': fpf_admin.rest_nonce,
			},
...
let getTagsOptions = (input) => {
	return fetch(
		fpf_admin.rest_url + 'flexible_product_fields/v1/tags/' + fpf_admin.rest_param + 'search=' + input,
		{
			headers: {
				'X-WP-Nonce': fpf_admin.rest_nonce,
			},
	...
	render() {
		return (
			<table className="fpf-table">
				...
				{ this.state.assign_to.value === 'tag' && fpf_assign_to_values[this.state.assign_to.value]['is_available'] ?
					<tr className="fpf-field fpf-field-tags">
						<td className="fpf-label">
							<label htmlFor="">{fpf_admin.tags_label}</label>
						</td>
