...
namespace VendorFPF\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use VendorFPF\WPDesk\Helper\Debug\LibraryDebug;
use VendorFPF\WPDesk\Helper\Integration\LicenseIntegration;
use VendorFPF\WPDesk\Helper\Integration\LogsIntegration;
use VendorFPF\WPDesk\Helper\Integration\SettingsIntegration;
use VendorFPF\WPDesk\Helper\Integration\TrackerIntegration;
use VendorFPF\WPDesk\Helper\Logs\LibraryInfoProcessor;
use VendorFPF\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \VendorFPF\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $helper_info = new \VendorFPF\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \VendorFPF\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \VendorFPF\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \VendorFPF\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \VendorFPF\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \VendorFPF\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \VendorFPF\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \VendorFPF\WPDesk\Helper\Debug\LibraryDebug();
        (new \VendorFPF\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \VendorFPF\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
